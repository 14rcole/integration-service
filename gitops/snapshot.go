...
func PrepareSnapshotForPipelineRun(pipelineRun *tektonv1.PipelineRun, component *applicationapiv1alpha1.Component, componentGroup *v1beta2.ComponentGroup) (*applicationapiv1alpha1.Snapshot, error) {
	...
	groupComponents, err := a.loader.GetAllComponentsInComponentGroup(a.context, a.client, componentGroup)
	if err != nil {
		return nil, err
	}
	...
	prefixes := []string{BuildPipelineRunPrefix, TestLabelPrefix, CustomLabelPrefix, ReleaseLabelPrefix}
	...
	if IsSnapshotCreatedByPACMergeQueueEvent(snapshot) {
		...
		if pullRequestNumber != "" {
			snapshot.Labels[PipelineAsCodePullRequestAnnotation] = pullRequestNumber
			snapshot.Annotations[PipelineAsCodePullRequestAnnotation] = pullRequestNumber
		}
	}
	...
	snapshot.Annotations[BuildPipelineRunStartTime] = strconv.FormatInt(timestampMillis, 10)
